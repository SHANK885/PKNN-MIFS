...
import numpy
from knn.utils import safe_asarray
...
def euclidean_distances(X, Y=None, inverse=False):
    """
    ...
    #print("NCOLS - solution_rows:", X.shape[0])
    #print("NROWS - solution_cols:", Y.shape[0])
    ...
    Y = numpy.ascontiguousarray(Y, dtype=numpy.float32)
    test_instance_gpu = gpuarray.to_gpu(Y)
    train_set_gpu = gpuarray.to_gpu(X)

    mod = SourceModule(kernel_code)
    func = mod.get_function("euclidean")
    func(drv.In(X), drv.In(Y), drv.Out(solution), block=(BLOCK_SIZE, BLOCK_SIZE, 1), grid=gdim)

    #func(train_set_gpu, test_instance_gpu, solution,
